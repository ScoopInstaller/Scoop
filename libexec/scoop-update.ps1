...
function update_scoop() {
    ...
    if (!(Test-Path "$currentdir\.git")) {
        ...
        $previousCommit = Invoke-Command { git -C "$currentdir" rev-parse HEAD }
        $currentRepo = Invoke-Command { git -C "$currentdir" config remote.origin.url }
        ...
        if ($isRepoChanged) {
            Invoke-Command { git -C "$currentdir" config remote.origin.url "$configRepo" }
        }

        # Fetch and reset local repo if the repo or the branch is changed
        if ($isRepoChanged -or $isBranchChanged) {
            # Reset git fetch refs, so that it can fetch all branches (GH-3368)
            Invoke-Command { git -C "$currentdir" config remote.origin.fetch '+refs/heads/*:refs/remotes/origin/*' }
            # fetch remote branch
            ...
            # reset branch HEAD
            Invoke-Command { git -C "$currentdir" reset --hard origin/$configBranch -q }
        } else {
        ...
        $res = $lastexitcode
        if ($show_update_log) {
            Invoke-Command { git -C "$currentdir" --no-pager log --no-decorate --grep='^(chore)' --invert-grep --format='tformat: * %C(yellow)%h%Creset %<|(72,trunc)%s %C(cyan)%cr%Creset' "$previousCommit..HEAD" }
        }
    ...
    foreach ($bucket in Get-LocalBucket) {
        ...
        $previousCommit = (Invoke-Command { git -C "$bucketLoc" rev-parse HEAD })
        git_cmd -C "`"$bucketLoc`"" pull -q
        if ($show_update_log) {
            Invoke-Command { git -C "$bucketLoc" --no-pager log --no-decorate --grep='^(chore)' --invert-grep --format='tformat: * %C(yellow)%h%Creset %<|(72,trunc)%s %C(cyan)%cr%Creset' "$previousCommit..HEAD" }
        }
    }
