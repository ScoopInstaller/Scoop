...
function find_hash_in_rdf([String] $url, [String] $useragent, [String] $basename) {
    ...
    try {
        ...
        $wc.Headers.Add('User-Agent', $(if($useragent) { $useragent } else { (Get-UserAgent) }))
...
function find_hash_in_headers([String] $url, [String] $useragent) {
    ...
    try {
        ...
        $req.UserAgent = $(if($useragent) { $useragent } else { (Get-UserAgent) })
        $req.Timeout = 2000
        $req.Method = 'HEAD'
        $res = $req.GetResponse()
...
function get_hash_for_app([String] $app, $config, [String] $version, [String] $url, [String] $useragent, [Hashtable] $substitutions) {
    ...
    switch ($hashmode) {
        ...
        'github' {
            ...
            $hash = find_hash_in_json $hashfile_url $useragent $substitutions ("$..assets[?(@.browser_download_url == '" + $originurl + "')].digest")
        }
    }
            ...
            if ($currentProperty -eq 'hash') {
                ...
                if ($Manifest.hash) {
                    ...
                    $Manifest.architecture | Get-Member -MemberType NoteProperty | ForEach-Object {
                        ...
                        $useragent = $(if($useragent) { $useragent } else { if($null -ne $Manifest.autoupdate.useragent) { $Manifest.autoupdate.useragent } else { if($null -ne $Manifest.checkver.useragent) { $Manifest.checkver.useragent } else { $null } }})
