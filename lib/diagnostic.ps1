...
function check_windows_defender($global) {
    ...
    if (Test-CommandAvailable Get-MpPreference) {
        # Get-MpPreference fails (e.g. 0x800106ba) when Defender is disabled by a third-party antivirus
        $preference = Get-MpPreference -ErrorAction SilentlyContinue
        if (!$preference -or $preference.DisableRealtimeMonitoring) { return $true }
