        ...
        if($extract_fn) {
            ...
            if(test-path "$dir\_tmp") { # might have been moved by movedir
                try {
                    ...
                    & "$env:WINDIR\System32\cmd.exe" /c "rmdir /s /q $dir\_tmp"
...
function link_current($versiondir) {
    ...
    if(test-path $currentdir) {
        ...
        & "$env:WINDIR\System32\cmd.exe" /c rmdir $currentdir
    }

    & "$env:WINDIR\System32\cmd.exe" /c mklink /j $currentdir $versiondir | out-null
    ...
    if($persist) {
        ...
        $persist | % {
            ...
            if (is_directory $target) {
                & "$env:WINDIR\System32\cmd.exe" /c "mklink /j `"$source`" `"$target`"" | out-null
                attrib $source.FullName +R /L
            } else {
                & "$env:WINDIR\System32\cmd.exe" /c "mklink /h `"$source`" `"$target`"" | out-null
            }
        }
    }
}
